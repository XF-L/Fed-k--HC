...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_path", type=str, default="none",help="input data path") #数据路径
    ...
    # args.data_path="../mufc-main/landsatpkl.pkl"
    ...
    for i_trail in range(args.num_trials):
        ...
        mufcTime = our_removal_time[i_trail, 0]
...
arr = np.array(idx)
print('ari {:.4f}'.format(adjusted_rand_score(dataset['true_label'],arr)), '\nnmi {:.4f}'.format(normalized_mutual_info_score(dataset['true_label'],arr)))
print("\n")

result = clustering_evaluate(dataset['true_label'],arr)
for key,value in result.items():
    print(f'{key}:{value}')

print(
            f"mufc time used: {mufcTime} s"
        )